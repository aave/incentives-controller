...
import {
  ...
  AToken,
  AToken__factory,
  IncentiveUpdateExecutor__factory,
...
describe('Enable incentives in target assets', () => {
  ...
  before(async () => {
    ...
    } = await new IncentiveUpdateExecutor__factory(proposer).deploy();
  ...
  it('Proposal should be created', async () => {
    ...
    await advanceBlockTo((await latestBlock()) + VOTING_DURATION);
  });
  ...
  it('User should be able to claim rewards after rewards are over', async () => {
    const incentives = StakedTokenIncentivesController__factory.connect(incentivesProxy, proposer);
